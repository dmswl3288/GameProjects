...
public class PopUp2 : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (ColorChange2._score < 0 && _popUpApprearCount == 0)
        {
            // 실패 팝업창 띄우기
            ...
            gameObject.GetComponent<ColorChange2>().DisableSquareButton();
        }

        // LV.4에서는 ViewMgr 코드 수정해야한다!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

        if (ColorChange2._numBlue == 8)  // 2단계는 8개 선택!!!!
        {
            if (ColorChange2._score < 16 && _curScorePopUpCount == 0)  //clear 했지만 만점은 아닐때
            {
                // 현재 점수 팝업창 띄우기!!!
            ...
            if (ColorChange2._score == 16 && _levelUpPopUpCount == 0)   //점수가 만점일때
            {
                //  LevelUP(LV.4) 팝업창 띄우기!!!
                ...
                _levelUpPopUp.transform.Find("NextLevelLv3").GetComponent<UnityEngine.UI.Button>().onClick.AddListener(delegate { NextLevelLv2ButtonClicked(); });
    ...
    // 3단계로 넘어가는 함수
    void NextLevelLv2ButtonClicked()
    {
    ...
    void ReStartButtonClicked()
    {
        ...
        // 점수 저장 초기화
        ColorChange2._score = 0;
        // 클리어 판별을 위한 블루버튼개수 
        ColorChange2._numBlue = 0;

        // 스퀘어 모두 흰색으로 원위치시키는 함수 호출
        gameObject.GetComponent<ColorChange2>().AllBackWhite();
